...
type PoolOptions struct {
	...
	// ChromePath overrides the browser executable path.
	// If empty, gorender auto-detects a sensible default for the host OS.
	ChromePath string
...
func (p *Pool) launch(parentCtx context.Context, id int) (*Browser, error) {
	...
	if p.opts.ChromePath != "" {
		allocOpts = append(allocOpts, chromedp.ExecPath(p.opts.ChromePath))
	}
	...
	browserCtx, browserCancel := chromedp.NewContext(allocCtx,
		chromedp.WithLogf(func(format string, args ...interface{}) {
			...
			// Chrome 145 emits this CDP enum warning frequently with older decoders.
			// It is noisy and non-fatal, so drop it from logs.
			if strings.Contains(msg, "unknown IPAddressSpace value: Loopback") {
				return
			}
			p.log.Sugar().Debugf("[chrome:%d] %s", id, msg)
		}),
	)
...
func detectChromePath() string {
	if fromEnv := os.Getenv("GORENDER_CHROME_BIN"); fromEnv != "" {
		return fromEnv
	}
