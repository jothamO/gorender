...
func TestReorderBuffer_InOrder(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		rb.Push(i, i, fmt.Sprintf("frame-%d.png", i), nil)
	}
...
func TestReorderBuffer_ConcurrentPush(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		...
		go func(frame int) {
			defer wg.Done()
			rb.Push(frame, frame, fmt.Sprintf("frame-%06d.png", frame), nil)
		}(i)
	}
...
func TestReorderBuffer_ChannelClosed(t *testing.T) {
	...
	rb.Push(0, 0, "a.png", nil)
	rb.Push(1, 1, "b.png", nil)
	rb.Push(2, 2, "c.png", nil)
