...
func (c *AssetCache) EnableInterception(browserCtx context.Context) error {
	...
	if err := chromedp.Run(browserCtx, fetch.Enable(nil)); err != nil {
	...
	chromedp.ListenTarget(ctx, func(ev interface{}) {
		...
		go func(p *fetch.EventRequestPaused) {
			...
			if entry != nil {
				...
				if err := chromedp.Run(ctx, params.Do); err != nil {
...
func (c *AssetCache) captureResponse(ctx context.Context, p *fetch.EventRequestPaused) {
	...
	err := chromedp.Run(ctx, chromedp.ActionFunc(func(ctx context.Context) error {
		var err error
		body, base64Encoded, err = fetch.GetResponseBody(p.RequestID).Do(ctx)
		return err
	}))
	...
	c.Put(p.Request.URL, body, contentType)
}
